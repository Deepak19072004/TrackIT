...
const TaskListTable = ({tableData}) => {
    ...
    const getPriColor = (priority) => {
        switch (priority) {
            ...
            case "Low": return 'bg-cyan-100 text-cyan-500 border border-cyan-200';
            default: return "bg-gray-100 text-gray-500 border border-gray-200";
        }
    };
  return (
                ...
                {tableData.map((task)=>(
                    <tr key={task._id} className='border-t border-gray-200'>
                        ...
                        <td className='py-4 px-4'>
                            <span className={`px-2 py-1 rounded inline-block  ${getstatuscolor(task.status)}`}>{task.status}</span>
                        </td>
                        <td className='py-4 px-4'>
                            <span className={`px-2 py-1 rounded inline-block  ${getPriColor(task.priority)}`}>{task.priority}</span>
                        </td>
